...
const FAQContent = () => {
  ...
  const faqs = [
    {
      question: translate("Do I need a registered business to apply?"),
      urduQuestion: "Kya apply karne ke liye mujhe registered business ki zaroorat hai?",
      answer: translate("No. Anyone with an idea can apply—students, workers, freelancers, creators. We believe great ideas can come from anywhere."),
    ...
    {
      question: translate("How many people will win?"),
      urduQuestion: "Kitne log jeetenge?",
      answer: translate("At least one winner will be selected per cycle, receiving the 10 Lakh Rupees. However, we also spotlight other top ideas for future opportunities and mentorship."),
    ...
    },
    {
      ...
      answer: translate("Doesn’t matter. We value impact and innovation more than the perceived 'size' of an idea. Small ideas can lead to big changes."),
    ...
    {
      question: translate("What kind of ideas are you looking for?"),
      urduQuestion: "Aap kis qism ke afkaar talash kar rahe hain?",
      answer: translate("We're looking for innovative, impactful, and feasible ideas that can make a difference. Whether it's a tech solution, a social enterprise, a creative project, or a local business, we want to hear it!"),
          ...
          <Accordion type="single" collapsible className="w-full">
            {faqs.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`}>
                <AccordionTrigger className="flex flex-col items-start text-lg sm:text-xl font-semibold text-green-700 hover:no-underline">
                  <span className="w-full text-left">{faq.question}</span>
                  <span className="w-full text-right text-sm sm:text-lg italic opacity-90 text-gray-600 mt-1">{faq.urduQuestion}</span>
                </AccordionTrigger>
