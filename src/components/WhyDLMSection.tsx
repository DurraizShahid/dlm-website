"use client";

import React from "react";

const WhyDLMSection = () => {
  return (
    <section className="bg-white text-gray-900 py-16 px-4 sm:py-24 sm:px-8 text-center">
      <div className="max-w-3xl mx-auto space-y-6">
        <h2 className="text-3xl sm:text-5xl font-extrabold leading-tight tracking-tight">
          We’re Here for Dreamers Nobody Else Believes In.
        </h2>
        <p className="text-lg sm:text-xl text-gray-700">
          Not everyone has family connections, investors, or startup backing. DLM exists for people like you—students, hustlers, creators, innovators—who just need one chance.
        </p>
        <p className="text-xl sm:text-2xl font-bold text-green-600">
          This isn’t charity. This is opportunity.
        </p>
      </div>
    </section>
  );
};

export default WhyDLMSection;